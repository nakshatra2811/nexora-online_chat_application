...
export const APP_URL = process.env.NEXT_PUBLIC_APP_URL || 'https://nexora-online-chat-application-liart.vercel.app';
export const APP_DOMAIN = 'nexora-online-chat-application-liart.vercel.app';
...
export async function nexoraFetch(endpoint: string, options: RequestInit = {}, retries = 1) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}${endpoint}`, {
      ...
      signal: AbortController ? AbortSignal.timeout(60000) : undefined,
    } as any);
