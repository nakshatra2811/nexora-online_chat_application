...
export function UserProfileModal({ friend, isDark, onClose, onChat, onVoiceCall, onVideoCall, onBlock, isChatLocked, onToggleLock, connectionStatus: initialConnectionStatus = "none", requestId, onConnectionChange }: UserProfileModalProps) {
  ...
  const [profileUrl, setProfileUrl] = useState(`https://nexora-online-chat-application-liart.vercel.app/auth?connect=${friend.username}`);
  const [displayUrl, setDisplayUrl] = useState(`https://nexora-online-chat-application-liart.vercel.app/auth?connect=********`);
